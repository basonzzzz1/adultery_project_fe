export const LOGIN = `/User/login`;
export const REGISTER = `/User/register`;
export const LOGOUT = `http://localhost:8080/User/logout`;
export const BANNER = `http://localhost:8080/User/banner`;
export const UNBANNER = `http://localhost:8080/User/unBanner`;
export const getAllCskhInUserAdmin = `http://localhost:8080/Chat/getAllCskhInUser`;
export const getAllCskhInUserDetail = `http://localhost:8080/Chat/getAllCskhInUserDetail`;
export const getAllCskhUser = `http://localhost:8080/Chat/getAllCskhUser`;
export const createCskhAdmin = `http://localhost:8080/Chat/createCskhAdmin`;
export const createCskh = `http://localhost:8080/Chat/createCskh`;
export const extraPointsInUser = `http://localhost:8080/User/extraPointsInUser`;
export const deleteChatRoom = `http://localhost:8080/Chat/deleteChatRoom`;
export const WEBSOCKETURL = `ws://localhost:8080/ws/websocket`;
export const CountSpin = `http://localhost:8080/User/countSpin`;
export const SpinRequest = `http://localhost:8080/User/editSpin`;
export const getListValueSpin = `http://localhost:8080/Chat/getValueSpin`;
export const setListValueSpin = `http://localhost:8080/Chat/setListValueSpin`;
export const getAllExchangePoints = `http://localhost:8080/Chat/getAllExchangePoints`;
export const createExchangePoints = `http://localhost:8080/Chat/createExchangePoints`;
export const deleteExchangePoints = `http://localhost:8080/Chat/deleteExchangePoints`;
export const FINDALLUSER = `http://localhost:8080/User/findAllUser`;
export const extraPoints = `http://localhost:8080/User/extraPoints`;
export const loggedInUser = `http://localhost:8080/User/loggedInUser`;
export const minusPoints = `http://localhost:8080/User/minusPoints`;
export const REQUESTCHATROOM = `http://localhost:8080/Chat/notification/create/chatroom`;
export const CREATECHATROOM = `http://localhost:8080/Chat/create/chatRoom`;
export const GETALLCHATROOM = `http://localhost:8080/Chat/chatRoom/all`;
export const FINDBYCHATROOMID = `http://localhost:8080/Chat/findByChatRoomId`;
export const CHECKCHATROOM = `http://localhost:8080/Chat/checkChatRoom`;
export const REQUESTALL = `http://localhost:8080/Chat/messageRequest/all`;
export const FINDALLMESSGEINCHATROOMID = `http://localhost:8080/Chat/findAllMessageInChatRoomId`;
export const CREATEMESSAGE = `http://localhost:8080/Chat/create/message`;